...
class Gemeinde:
    def __init__(self, id, name, ars_land,ars_rb,ars_kreis,ars_vb,ars_gem,flaeche,postleitzahl,bevoelkerung_insgesamt,bevoelkerung_maennlich,bevoelkerung_weiblich,bevoelkerung_pro_flaeche,ort_laengengrad,ort_breitengrad,verstaedterung):
        self.id = id
        self.name = name
        self.ars_land = ars_land
        self.ars_rb = ars_rb
...
with open(home_path+'regional_data.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in csv_reader:
        ...
        if row[0] == '10':
            ...
            gemeinden.append(Gemeinde(id,name,row[2],row[3],row[4],row[5],row[6],row[8],row[13],row[9],row[10],row[11],row[12],row[14],row[15],row[18]))
...
with open(home_path+'database/geo_regierungsbezirk.csv', 'w', newline='', encoding="utf-8") as file:
    ...
    for regierungsbezirk in regierungsbezirke:
        writer.writerow([regierungsbezirk.id, regierungsbezirk.name, regierungsbezirk.ars_land,regierungsbezirk.ars_rb])
...
with open(home_path+'database/geo_gemeindeverband.csv', 'w', newline='', encoding="utf-8") as file:
    ...
    for gemeindeverband in gemeindeverbaende:
        writer.writerow([gemeindeverband.id, gemeindeverband.name, gemeindeverband.ars_land,gemeindeverband.ars_rb,gemeindeverband.ars_kreis,gemeindeverband.ars_vb])

# Export Gemeinden
with open(home_path+'database/geo_gemeinde.csv', 'w', newline='', encoding="utf-8") as file:
    writer = csv.writer(file)
    writer.writerow(["id", "name", "ars_land","ars_rb","ars_kreis","ars_vb","ars_gem","flaeche","postleitzahl","bevoelkerung_insgesamt","bevoelkerung_maennlich","bevoelkerung_weiblich","bevoelkerung_pro_flaeche","ort_laengengrad","ort_breitengrad","verstaedterung"])

    for gemeinde in gemeinden:
        writer.writerow([gemeinde.id, gemeinde.name, gemeinde.ars_land,gemeinde.ars_rb,gemeinde.ars_kreis,gemeinde.ars_vb,gemeinde.ars_gem,gemeinde.flaeche,gemeinde.postleitzahl,gemeinde.bevoelkerung_insgesamt.replace(" ",''),gemeinde.bevoelkerung_maennlich.replace(" ",''),gemeinde.bevoelkerung_weiblich.replace(" ",''),gemeinde.bevoelkerung_pro_flaeche.replace(" ",''),gemeinde.ort_laengengrad,gemeinde.ort_breitengrad,gemeinde.verstaedterung])